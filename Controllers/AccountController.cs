...
namespace Academia.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var objPessoa = db.Pessoa.Where(a => a.Email == model.Email).FirstOrDefault();
        ...
        public ActionResult Register()
        {
            ...
            int count = 0;
            count = db.Pessoa.Where(a => a.TipoPessoa.ToString() == "Administrador").Count();
            ViewData["AdminExiste"] = count;
            return View();
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                //var x = await UserManager.FindByIdAsync(user.Id); //use async find
                //var res = await UserManager.DeleteAsync(x);

                if (result.Succeeded)
                {
                    var objPessoa = db.Pessoa.Add(new Pessoa()
                    {      
                        Codigo = 0,
                        Email = model.Email,
                        Nome = model.Nome,
                    ...
                    var obPessoa = db.Pessoa.Where(a => a.Email == model.Email).FirstOrDefault();
                        ...
                        foreach (XmlElement element in xmlDoc.DocumentElement)
                        {
                            if (element.Name.Equals("appSettings"))
                            {
                                foreach (XmlNode node in element.ChildNodes)
                                {
                                    if (node.Attributes[0].Value.Equals("TipoUsuario"))
                                    {
                                        node.Attributes[1].Value = obPessoa.TipoPessoa.ToString();
                                    }
                                }
                            }
                        }
